...
import pv3199.lib.java.math.Fraction;
...
public final class RealMatrix extends NumberMatrix<Double> {
	 ...
	 */
	public RealMatrix(Number[][] initialData) throws NullPointerException, IllegalArgumentException, ClassCastException {
		// this(pvlib.util.Arrays.castArray(initialData, Double[][].class));
		super(pv3199.lib.java.util.Arrays.castArray(initialData, Double[][].class));
	...
	@Override
	public Double subDeterminant(int startRow, int startColumn, int endRow, int endColumn) {
		Double[][] data = new Double[endRow - startRow][endColumn - startColumn];
		
		for (int r = startRow; r < endRow; r++) {
			for (int c = startColumn; c < endColumn; c++) {
				data[r - startRow][c - startColumn] = this.getValue(r, c);
			}
		}
		
		return determinant0(data);
	}
