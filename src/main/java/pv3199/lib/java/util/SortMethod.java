...
public enum SortMethod {
	...
	MERGE(arg -> {
		...
		Function<DataStructure<?>[], DataStructure<Object>> merge = args -> {
			...
			while (!first.isEmpty() || !second.isEmpty()) {
				...
				int compareResult = firstElement == null ? 1
						: secondElement == null ? -1 : comparator.compare(firstElement, secondElement);
	 ...
	 * 
	 * @param algorithm
	 *            - the algorithm function used for sorting the data.
	 */
